...
class SlotsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create slot" do
    assert_difference("Slot.count") do
      post slots_url, params: { slot: { parking_id: @slot.parking_id, status: @slot.status, type: @slot.type, zone: @slot.zone } }
    end
  ...
  test "should update slot" do
    patch slot_url(@slot), params: { slot: { parking_id: @slot.parking_id, status: @slot.status, type: @slot.type, zone: @slot.zone } }
